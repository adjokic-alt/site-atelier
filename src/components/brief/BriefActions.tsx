...
export function SelectStyleButton({ styleId, children }: { styleId: StyleId; children: ReactNode }) { const { brief, dispatch } = useBrief(); const selected = brief.style.primaryStyleId === styleId; return <Button variant="style" size="lg" onClick={() => dispatch({ type: "SET_PRIMARY_STYLE", styleId })}>{selected ? "Selected for my brief" : children}</Button>; }
