...
export type BriefAction =
  | { type: "HYDRATE"; brief: ClientBrief }
  | { type: "RESET"; brief: ClientBrief }
  | { type: "SET_PRIMARY_STYLE"; styleId: StyleId | null }
  | { type: "SET_ACCENT_STYLE"; styleId: StyleId | null }
  | { type: "TOGGLE_MATERIAL"; materialId: string }
  | { type: "SET_PROJECT_CATEGORIES"; categoryIds: ProjectCategoryId[] }
  | { type: "SET_LOCATION"; country: string; city?: string; postcode?: string }
  | { type: "SET_BUDGET"; currency: CurrencyCode | null; rangeId: string | null }
  | { type: "ADD_MOODBOARD_ITEM"; itemId: string }
  | { type: "REMOVE_MOODBOARD_ITEM"; itemId: string }
  | { type: "SET_CONTACT"; name?: string; email?: string; phone?: string | null };

function touch(brief: ClientBrief): ClientBrief { return { ...brief, updatedAt: new Date().toISOString() }; }

export function briefReducer(state: ClientBrief, action: BriefAction): ClientBrief {
  switch (action.type) {
    case "HYDRATE": return action.brief;
    case "RESET": return action.brief;
    ...
    case "TOGGLE_MATERIAL": { const selected = state.style.materialIdsLiked.includes(action.materialId); return touch({ ...state, style: { ...state.style, materialIdsLiked: selected ? state.style.materialIdsLiked.filter((id) => id !== action.materialId) : [...state.style.materialIdsLiked, action.materialId] } }); }
    ...
    case "SET_BUDGET": return touch({ ...state, budget: { ...state.budget, currency: action.currency, rangeId: action.rangeId } });
